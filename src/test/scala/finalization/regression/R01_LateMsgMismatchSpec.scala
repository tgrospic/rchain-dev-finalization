...
class R01_LateMsgMismatchSpec extends FlatSpec with Matchers {
  ...
  val sut = new NetworkRunner[Task](enableOutput = false)
...
//    println(a.mkString("\n"))
  ...
  private def regressionTest[F[_]: Sync](
    ...
    for {
      ...
      n_167755778_                <- runSections(n1022813136, List((1, 0.6574136f)), "n1022813136")
      ...
      n_936033305_                <- runSections(n171242518, List((8, 0.8514173f)), "n171242518")
      ...
      n2137437950_                <- runSections(n1502918575, List((7, 0.8242769f)), "n1502918575")
      (n2137437950, _)             = n2137437950_
      n1281978524_                <- runSections(n_36996899, List((8, 0.67792714f)), "n_36996899")
      ...
      n221340169_                 <- runSections(n_1870652816, List((6, 0.707146f)), "n_1870652816")
      ...
      n830674740_                 <- runSections(n123943753, List((4, 0.053336143f)), "n123943753")
      (n830674740, _)              = n830674740_
      n1773789275                  = n830674740 >|< n_950423999
      (n_12835555, n_1845453989)   = n1773789275.split(0.6123032f)
      n1552341417_                <- runSections(n_12835555, List((10, 0.7348425f)), "n_12835555")
      ...
      n2118929613_                <- runSections(n716266525, List((3, 0.085235894f)), "n716266525")
      (n2118929613, _)             = n2118929613_
      n_328185564_                <- runSections(n285850228, List((10, 0.84769547f)), "n285850228")
      ...
      n_1585794645_               <- runSections(n80197349, List((13, 0.16910058f)), "n80197349")
      ...
      n_188358953_                <- runSections(n826889543, List((2, 0.8328306f)), "n826889543")
      ...
      n995028883_                 <- runSections(n1671848291, List((8, 0.77417606f)), "n1671848291")
      (n995028883, _)              = n995028883_
      (n_1269818380, n335670858)   = n_188358953.split(0.08835584f)
      r                           <- runSections(n_1269818380, List((8, 0.07683784f)), "n_1269818380")
    } yield r
  }
}
