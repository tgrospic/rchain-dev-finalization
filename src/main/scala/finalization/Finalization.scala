...
object Finalization {
      ...
      if (dag.contains(msg.id)) this
        ...
        val iterSeen    = msgJfs.map { rootMsg =>
          ...
          rootMsg.sender -> Iterator
            .iterate((seerViews, seerViews)) { case (acc, jfs) =>
        ...
        val seenBySeen = mViews
          .foldLeft(Map[Msg, Set[MsgView]]()) { case (acc, mv) =>
  ...
  final case class Network(senders: Set[SenderState]) {
    ...
    def split(groups: Seq[Int]) = {
      val (rest, nets) = groups.foldLeft((senders, Vector[Network]())) { case ((acc, r), n) =>
        val (sendersInGroup, rest) = acc.splitAt(n)

        assert(sendersInGroup.nonEmpty, "Network has no senders")

        val net = Network(sendersInGroup)
        (rest, r :+ net)
      }
      if (rest.nonEmpty) {
        nets :+ Network(rest)
